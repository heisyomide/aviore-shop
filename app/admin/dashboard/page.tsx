...
import { 
  Plus, Package, Upload, User, CheckCircle, Archive, 
  ShieldCheck, Mail, Phone, Trash2, Search, TrendingUp, 
  Layers, CreditCard, RefreshCw, Hash
} from "lucide-react";
import { 
...
interface OrderItem {
  ...
  customerPhone: string; // Ensure this matches your checkout field name
  amount: number;
  status: string;
  tx_ref: string;
  items: InventoryItem[];
...
export default function AdminDashboard() {
  ...
  const stats = useMemo(() => {
    // 1. Revenue: Only sum orders marked as SUCCESS
    const totalRevenue = orders
      .filter(o => o.status === "SUCCESS")
      .reduce((sum, order) => sum + order.amount, 0);
    
    // 2. Live Inventory: Items in DB that are NOT sold
    const liveCount = inventory.filter(i => !i.isSold).length;

    // 3. Successful Transfers: Total count of items that have been sold
...
const handleStatusUpdate = async (id: string) => {
  ...
  try {
    ...
    if (res.success) {
      // THIS LINE IS THE KEY. It forces the dashboard to see the new "SOLD" status.
      await refreshData(); 
      alert("ACQUISITION_VERIFIED: Piece is now marked as SOLD.");
    }
  } finally {
    setLoading(false);
  }
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-mono p-4 md:p-8 selection:bg-white selection:text-black">
      ...
      <main className="max-w-7xl mx-auto space-y-12">
        ...
        {activeTab === "inventory" && (
          <section className="grid grid-cols-1 lg:grid-cols-12 gap-12 animate-in fade-in slide-in-from-left-4 duration-500">
            {/* LEFT: UPLOAD FORM */}
            <div className="lg:col-span-4 space-y-6">
              <h2 className="text-[10px] uppercase tracking-[0.4em] text-white/40 flex items-center gap-2">
                <Plus size={14} /> Commit_New_Piece
              </h2>
              <form onSubmit={handleCommit} className="space-y-4 bg-white/2 border border-white/5 p-6">
                <select name="category" required className="w-full bg-black border border-white/10 p-3 text-[10px] outline-none text-white uppercase focus:border-white/40">
                  <option value="">Target_Category</option>
                ...
                <div className="space-y-2">
                  ...
                  <div onClick={() => fileInputRef.current?.click()} className="border border-dashed border-white/10 h-32 flex flex-col items-center justify-center gap-2 cursor-pointer hover:bg-white/3 transition-all overflow-hidden relative group">
                    {selectedImage ? (
                    ...
                    <tbody className="text-[9px] uppercase">
                      {inventory.map((item) => (
                        <tr key={item._id} className={`border-b border-white/5 hover:bg-white/2 transition-colors ${item.isSold ? 'opacity-30' : ''}`}>
                          <td className="p-4 text-white/40 font-mono tracking-tighter">{item.lotNumber || 'UNTITLED'}</td>
                          <td className="p-4">
                             <span className="block font-bold tracking-widest">{item.name}</span>
                             <span className="text-[7px] text-white/30 tracking-[0.2em]">₦{item.price.toLocaleString()}</span>
                          </td>
                          <td className="p-4 text-right">
                            <div className="flex items-center justify-end gap-3">
                              <span className={`px-2 py-0.5 border text-[7px] tracking-widest font-bold ${item.isSold ? 'border-red-500/20 text-red-500 bg-red-500/5' : 'border-green-500/20 text-green-500 bg-green-500/5'}`}>
            ...
            <div className="flex justify-between items-center border-b border-white/5 pb-6">
              ...
              <div className="relative w-64">
                ...
                <input 
                  ...
                  className="w-full bg-white/3 border border-white/10 py-2 pl-9 pr-4 text-[9px] outline-none focus:border-white/30"
                  value={searchQuery}
            ...
            <div className="grid grid-cols-1 gap-4">
              {filteredOrders.length === 0 ? (
                ...
                filteredOrders.map((order) => (
                  <div key={order._id} className="bg-white/1 border border-white/5 p-6 flex flex-col md:flex-row justify-between gap-8 group hover:border-white/20 transition-all relative overflow-hidden">
                    
                    {/* CUSTOMER ENTITY (WITH PHONE) */}
                    <div className="space-y-4 min-w-55">
                      <div className="space-y-2">
                        <p className="text-[8px] text-white/20 uppercase tracking-[0.2em] font-bold">Customer_Entity</p>
                        ...
                        <div className="space-y-1 pt-1 border-t border-white/5">
                          <p className="text-[9px] text-white/40 flex items-center gap-2"><Mail size={10}/> {order.customerEmail}</p>
                          <p className="text-[10px] text-green-500 font-bold flex items-center gap-2"><Phone size={10}/> {order.customerPhone || 'NO_CONTACT'}</p>
                        </div>
                      </div>
                      <div className={`text-[7px] inline-block px-2 py-1 border font-bold tracking-widest ${order.status === 'SUCCESS' ? 'border-green-500/30 text-green-500 bg-green-500/5' : 'border-yellow-500/30 text-yellow-500'}`}>
                    ...
                    <div className="flex-1 md:border-x border-white/5 md:px-8 flex flex-col justify-center gap-3">
                      <p className="text-[8px] text-white/20 uppercase tracking-[0.2em] font-bold mb-1">Acquired_Specimens</p>
                      {order.items.map((item) => (
                        <div key={item._id} className="flex items-center gap-4 bg-white/2 p-2 border border-white/5">
                          <div className="h-8 w-8 bg-zinc-900 border border-white/10 grayscale shrink-0">
                          ...
                          <div>
                            ...
                            <p className="text-[7px] text-white/20 uppercase tracking-widest">{item.lotNumber || 'ARCHIVE'}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="text-right flex flex-col justify-between items-end min-w-45">
                      <div>
                        <p className="text-[18px] italic font-serif text-white">₦{order.amount.toLocaleString()}</p>
                        <p className="text-[8px] text-white/10 font-mono mt-1 uppercase flex items-center gap-1 justify-end">
                      ...
                      <button 
                        ...
                        className={`mt-4 w-full md:w-auto px-6 py-4 text-[9px] uppercase tracking-[0.4em] border font-bold transition-all ${order.status === 'SUCCESS' ? 'border-green-500/30 text-green-500 opacity-50' : 'border-white/10 hover:bg-white hover:text-black'}`}
                      >
  ...
  return (
    <div className="p-6 border border-white/5 bg-white/1 flex flex-col justify-between hover:bg-white/2 transition-all h-32 group">
      <div className="flex justify-between items-start text-white/20 group-hover:text-white/40 transition-all">
        <span className="text-[8px] uppercase tracking-[0.3em] font-bold">{label}</span>
        {icon}
      </div>
